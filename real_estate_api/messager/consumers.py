...
class MessagerConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.scope["user"].is_anonymous:
            ...
            await self.channel_layer.group_add("{}".format(self.scope['user']), self.channel_name)
            await self.accept()

    async def disconnect(self, close_code):
        """Consumer implementation to leave behind the group at the moment the
        closes the connection."""
        await self.channel_layer.group_discard("{}".format(self.scope['user']), self.channel_name)
